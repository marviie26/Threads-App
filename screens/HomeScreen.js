...
const HomeScreen = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://192.168.0.161:3000/get-posts");
  ...
  const handleLike = async (postId) => {
    try {
      const response = await axios.put(
        `http://192.168.0.161:3000/posts/${postId}/${userId}/like`
      );
  ...
  const handleDislike = async (postId) => {
    try {
      const response = await axios.put(
        `http://192.168.0.161:3000/posts/${postId}/${userId}/unlike`
      );
      const updatedPost = response.data;
